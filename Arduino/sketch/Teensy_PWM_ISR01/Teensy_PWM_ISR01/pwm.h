...
void PWMInit(uint32_t PERIPHERAL_BUS_CLOCK, uint8_t FTM0_CLK_PRESCALE, uint32_t FTM0_OVERFLOW_FREQUENCY, uint8_t FTM0_DEADTIME_DTVAL)
{
  ...
  */
  ...
  FTM0_C0SC |= FTM_CnSC_ELSB; //Edge or level select
  FTM0_C0SC &= ~FTM_CnSC_ELSA; //Edge or level Select
  FTM0_C0SC |= FTM_CnSC_MSB; //Channel Mode select

  // FTMx_CnSC - contains the channel-interrupt status flag control bits
  FTM0_C1SC |= FTM_CnSC_ELSB; //Edge or level select
  FTM0_C1SC &= ~FTM_CnSC_ELSA; //Edge or level Select
  FTM0_C1SC |= FTM_CnSC_MSB; //Channel Mode select

  // FTMx_CnSC - contains the channel-interrupt status flag control bits
  FTM0_C2SC |= FTM_CnSC_ELSB; //Edge or level select
  FTM0_C2SC &= ~FTM_CnSC_ELSA; //Edge or level Select
  FTM0_C2SC |= FTM_CnSC_MSB; //Channel Mode select

  // FTMx_CnSC - contains the channel-interrupt status flag control bits
  FTM0_C3SC |= FTM_CnSC_ELSB; //Edge or level select
  FTM0_C3SC &= ~FTM_CnSC_ELSA; //Edge or level Select
  FTM0_C3SC |= FTM_CnSC_MSB; //Channel Mode select

  // FTMx_CnSC - contains the channel-interrupt status flag control bits
  FTM0_C4SC |= FTM_CnSC_ELSB; //Edge or level select
  FTM0_C4SC &= ~FTM_CnSC_ELSA; //Edge or level Select
  FTM0_C4SC |= FTM_CnSC_MSB; //Channel Mode select

  // FTMx_CnSC - contains the channel-interrupt status flag control bits
  FTM0_C5SC |= FTM_CnSC_ELSB; //Edge or level select
  FTM0_C5SC &= ~FTM_CnSC_ELSA; //Edge or level Select
  FTM0_C5SC |= FTM_CnSC_MSB; //Channel Mode select
  ...
  FTM0_SC = FTM_SC_CLKS_SYSCLK; // Selects Clock source to be "system clock"
...
{
  // SWOCTRL Bit Fields
  // u,v,w durch Software steuern und mit LOW �berschreiben, damit x,y,z mittels PWM bremsen koennen
  FTM0_SWOCTRL |= FTM_SWOCTRL_CH1OC|FTM_SWOCTRL_CH3OC|FTM_SWOCTRL_CH5OC;
